...
namespace apriltag_cpp
{
    ...
    class ApriltagQuadsComponent : public rclcpp::Node
    {
    public:
        ApriltagQuadsComponent(const rclcpp::NodeOptions &options)
            : Node("apriltag_quads_component", options), family("Tag36h11"), params(), detector(family, params)
        {
            subscription_ = this->create_subscription<apriltag_utils::msg::HullsResult>(
                "/apriltag/hulls", 2, std::bind(&ApriltagQuadsComponent::topic_callback, this, std::placeholders::_1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(apriltag_cpp::ApriltagQuadsComponent)
