...
namespace apriltag_cpp
{
    ...
    class ApriltagHullsComponent : public rclcpp::Node
    {
    public:
        ApriltagHullsComponent(const rclcpp::NodeOptions &options)
            : Node("apriltag_hulls_component", options), family("Tag36h11"), params(), detector(family, params)
        {
            subscription_ = this->create_subscription<apriltag_utils::msg::ContoursResult>(
                "/apriltag/contours", 2, std::bind(&ApriltagHullsComponent::topic_callback, this, std::placeholders::_1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(apriltag_cpp::ApriltagHullsComponent)
