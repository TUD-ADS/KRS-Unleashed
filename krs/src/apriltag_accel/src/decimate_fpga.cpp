...
#include "apriltag_accel/decimate_fpga.hpp"
...
#define KERNEL_NAME "cvtcolor_rgb2gray"

namespace apriltag_accel
{

	void DecimateFPGA::InitKernel()
	{
		cl_int err;
	...
	void DecimateFPGA::ExecuteKernel()
	{
		int rows = HEIGHT;
		int cols = WIDTH;
	...
	DecimateFPGA::DecimateFPGA(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
		: rclcpp::Node("apriltag_decimate_fpga_node", options)
	{
		// Create image pub
		...
		subscriber_ = image_transport::create_subscription(
			this, "/image_raw",
			std::bind(&DecimateFPGA::imageCb, this, std::placeholders::_1),
			hints.getTransport());
		...
		TRACEPOINT(kernel_register,
				   ...
				   "decimate_kernel_fpga");
	}

	void DecimateFPGA::imageCb(const sensor_msgs::msg::Image::ConstSharedPtr &image_msg)
	{
	...
	size_t DecimateFPGA::get_msg_size(sensor_msgs::msg::Image::ConstSharedPtr image_msg)
	{
...
RCLCPP_COMPONENTS_REGISTER_NODE(apriltag_accel::DecimateFPGA)
