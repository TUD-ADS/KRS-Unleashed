...
#define KERNEL_NAME "adaptive_threshold"

namespace apriltag_accel
{

	void ThresholdFPGA::InitKernel()
	{
		cl_int err;
	...
	void ThresholdFPGA::ExecuteKernel()
	{
		int rows = HEIGHT;
		int cols = WIDTH;
	...
	ThresholdFPGA::ThresholdFPGA(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
		: rclcpp::Node("apriltag_threshold_fpga_node", options)
	{
		// Create image pub
		publisher_ = this->create_publisher<sensor_msgs::msg::Image>("/apriltag/thresholded", 10);
		image_transport::TransportHints hints(this, "raw");
		subscriber_ = image_transport::create_subscription(
			this, "/apriltag/decimated",
			std::bind(&ThresholdFPGA::imageCb, this, std::placeholders::_1),
			hints.getTransport());
		...
		TRACEPOINT(kernel_register,
				   ...
				   "threshold_kernel_fpga");
	}

	void ThresholdFPGA::imageCb(const sensor_msgs::msg::Image::ConstSharedPtr &image_msg)
	{
	...
	size_t ThresholdFPGA::get_msg_size(sensor_msgs::msg::Image::ConstSharedPtr image_msg)
	{
...
RCLCPP_COMPONENTS_REGISTER_NODE(apriltag_accel::ThresholdFPGA)
